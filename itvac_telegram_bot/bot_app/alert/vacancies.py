...
def get_new_vacancies():
    ...
    vacancies = rabota_az.vacancies() + day_az.vacancies() + \
        boss_az.vacancies() + azerjobs_com.vacancies()
    return vacancies
...
class __RabotaAz:
    ...
    def vacancies(self, url=url_daily):
        ...
        if page.status_code == 200:
            ...
            try:
                vacancies = soup.find(
                    'ul', {'class': 'visitor-vacancy-list'}).find_all('li')
            ...
            for vacancy in vacancies:
                ...
                company = vacancy.find(
                    'span', {'class': 'company-'}).b.get_text()
                # Overview is in the last <p>
                overview = vacancy.find_all('p')[-1].get_text()
                # [1:0] removes new line at the beginning
                overview = overview.replace("\t", '')[1:]
                url = self.base_url + \
                    vacancy.find('a', {'class': 'title-'}).get('href')
                data['results'].append(
                    {
...
class __DayAz:
    ...
    def vacancies(self, url=url_daily):
        ...
        if page.status_code == 200:
            ...
            try:
                vacancies = soup.find(
                    'div', {'class': 'vacancy-serp'}).find_all('div', {'class': 'vacancy-serp-item'})
            except AttributeError:
                return data['results']
            for vacancy in vacancies:
                title = vacancy.find(
                    'a', {'data-qa': 'vacancy-serp__vacancy-title'}).get_text()
                try:
                    salary = vacancy.find(
                        'div', {'data-qa': 'vacancy-serp__vacancy-compensation'}).get_text()
                except:
                    salary = "Not specified"
                location = vacancy.find(
                    'span', {'data-qa': 'vacancy-serp__vacancy-address'}).get_text()
                company = vacancy.find(
                    'a', {'data-qa': 'vacancy-serp__vacancy-employer'}).get_text()
                requirements = vacancy.find(
                    'div', {'data-qa': 'vacancy-serp__vacancy_snippet_requirement'}).get_text()
                responsibility = vacancy.find(
                    'div', {'data-qa': 'vacancy-serp__vacancy_snippet_responsibility'}).get_text()
...
class __BossAz:
    ...
    # Using english version of site to get date in right format
    base_url = "https://en.boss.az"
    base_url_az = "https://boss.az"

    # offset - vacancy post date period (e.g. 7 = for the past week, 0 - today)
    def vacancies(self, url=url, offset=0):
        ...
        if page.status_code == 200:
            ...
            try:
                ...
                for vac in all_vacancies:
                    link = vac.find(
                        'a', {'class': 'results-i-link'}).get('href')
                    inner_vac = requests.get(
                        self.base_url + link, headers={'User-agent': self.user_agent})
                    inner_soup = BeautifulSoup(inner_vac.content, 'lxml')
                    vac_date = inner_soup.find(
                        'div', {'class': 'bumped_on params-i-val'}).get_text()
            ...
            for vacancy in vacancies:
                ...
                salary = vacancy.find(
                    'div', {'class': 'results-i-salary'}).get_text()
                if salary == "—" or salary == "-":
                    salary = "Not specified"
                location = str(vacancy.find(
                    'div', {'class': 'results-i-secondary'}).contents[0])
                company = vacancy.find(
                    'a', {'class': 'results-i-company'}).get_text()
                overview = vacancy.find(
                    'div', {'class': 'results-i-summary'}).p.get_text()
                # Send user AZ version of site
                url = self.base_url_az + \
                    vacancy.find('a', {'class': 'results-i-link'}).get('href')
                data['results'].append(
                    {
                        "title": title, "salary": salary, "location": location,
                        "company": company, "overview": overview, "url": url
...
class __AzerjobsCom:
    ...
    # Get website in azerbaijani
    url_set_lang = "https://www.azerjobs.com/set-language/az"
        ...
        if page.status_code == 200:
            ...
            for link in vacancy_links:
                ...
                if vac_page.status_code == 200:
                    ...
                    title = vacancy.find(
                        'div', {'class': 'anncmt-title'}).h1.get_text()
                    details = vacancy.find(
                        'div', {'class': 'posting-details'}).find_all('table', {'class': 'firm-card'})
                    ...
                    if 'salary' not in locals():
                        salary = 'Not specified'
                    company = vacancy.find(
                        'div', {'class': 'anncmt-customer'}).h2.a.get_text().strip()
                    overview = vacancy.find(
                        'div', {'class': 'firm-descr'}).find_all('ul')[0].get_text().strip()
